...
export default function TaskCard({ task, onComplete, onStartVerification }: TaskCardProps) {
  ...
  const handleAction = () => {
    if (task.status === 'idle') {
      ...
      setIsVerifying(true);
      setCountdown(1); // 1-second simulation
    }
  };
          ...
          <div className="flex-1">
            <AnimatePresence mode="wait">
              ...
              {isVerifying && (
                <motion.div
                  key="btn-verifying"
                  className="w-full bg-zinc-950 border-2 border-zinc-850 px-3 py-2.5 rounded-xl flex items-center justify-center gap-2 text-xs text-zinc-300 font-bold"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                >
